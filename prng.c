...
int 
reseed_prng(struct prng_context *prng, const struct entropy_pool *pool, int param)
{
	unsigned char v0[MAXDIGEST], digest[MAXDIGEST]; 
	unsigned char val[4];
	int i, key;
	...
	for (i = 2; i <= param; i++) {
		hdesc->init(&ctx);
		hdesc->update(&ctx, digest, len);
		hdesc->update(&ctx, val, len);
		 
		val[0] = (i & 0xff000000) >> 24;
		val[1] = (i & 0xff0000) >> 16;
		val[2] = (i & 0xff00) >> 8;
		val[3] = (i & 0xff);
		hdesc->update(&ctx, val, sizeof(val));
		hdesc->finalize(&ctx, digest);
	}
	
	for (i = 0; i < ARRSZ(prng->counter); i++){
		prng->counter[i] = 0;			
	}
...
void prng_encrypt(struct prng_context *prng, void *buf, size_t *size)
{
	...
	while (size != 0){
		...
		*size<BLOCK_SIZE ? cpy_sz=*size : cpy_sz=BLOCK_SIZE;
		...
		prng_next(prng);			/*calculate the next value*/
